...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		sp := print.Spinner(os.Stdout, "Trash Path [%s] ...", newPath)
		ctx, cancel := context.WithTimeout(context.Background(), time.Second*30)
		defer cancel()
		result, err := pubUserFile.NewPubUserFileClient(serv.GetGrpcConnection()).Trash(ctx, &pubUserFile.BatchOperationRequest{
		...
		sp(true)
		print.SuccessStatusEvent(os.Stdout, "Trash operation [%s] created.", result.Task)
	},
}

func init() {
	DiskCmd.AddCommand(trashCmd)

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// trashCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	trashCmd.Flags().BoolP("force", "f", false, "Force remove, instead of put items into trash.")
}
